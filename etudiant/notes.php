...
if(isset($_SESSION['login'])){
   ...
   $etudiant=$connexion->query("SELECT * FROM note WHERE id_etudiant='$id_etudiant'");
   }
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <tbody>
        ...
        if($etudiant){ 
			...
			while($ligne=$etudiant->fetch()){
                ...
                $req23=$connexion->query("SELECT * FROM semestre WHERE id_semestre='$id_semestre'");
                $nom_semestre=$req23->fetch(PDO::FETCH_ASSOC);
		?>
		<tr>
            <td><?php echo $nom_matiere['nom_matiere']; ?></td>
            <td><?php echo $nom_semestre['nom_semestre'] ?></td>
